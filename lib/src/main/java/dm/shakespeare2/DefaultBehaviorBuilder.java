...
class DefaultBehaviorBuilder implements BehaviorBuilder {
  ...
  private static final Observer<Context> sDefaultContextObserver = new Observer<Context>() {

    public void accept(@NotNull final Context context) {
    }
  };

  private static final Handler<?> sDefaultMessageHandler = new Handler<Object>() {
  ...
  public Behavior build() {
    ...
    if (messageHandlers.isEmpty()) {
      if (noMatchHandlers.isEmpty()) {
        messageHandler = sDefaultMessageHandler;
    ...
    if (startObservers.isEmpty()) {
      startObserver = sDefaultContextObserver;
    ...
    if (stopObservers.isEmpty()) {
      stopObserver = sDefaultContextObserver;
