 ...
 * {@link dm.shakespeare.typed.annotation.ActorFrom ActorFrom} and
 * {@link dm.shakespeare.typed.annotation.HeadersFrom HeadersFrom} annotation respectively.
 * <br>
 ...
 * must expect an {@link dm.shakespeare.typed.message.InvocationResult InvocationResult} or
 * an {@link dm.shakespeare.typed.message.InvocationException InvocationException} messages
 * as response.
 */
public class TypedStage extends Stage {
  ...
  @NotNull
  public <T> List<T> findAll(@NotNull final Class<? extends T> type) {
    return Collections.unmodifiableList(ActorHandler.<T>getProxies(type, stage));
  }

  @Nullable
  public <T> T findAny(@NotNull final Class<? extends T> type) {
    return ActorHandler.getProxy(type, stage);
  }
}
